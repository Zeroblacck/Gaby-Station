...
namespace Content.Server._Gabystation.Speech.EntitySystems
{
    public sealed class ProfanityFilterAccentSystem : EntitySystem
    {
        ...
        {
            SubscribeLocalEvent<ProfanityFilterAccentComponent, AccentGetEvent>(OnAccent);
        }

        private void OnAccent(EntityUid uid, ProfanityFilterAccentComponent component, AccentGetEvent args)
        {
            args.Message = ApplyReplacements(args.Message, component.Words, component.WordReplacement);
        }
        ...
        /// </summary>
        [PublicAPI]
        public string ApplyReplacements(string message, string accent, string replacement_id)
        {
            if (!_proto.TryIndex<ProfanityFilterAccentPrototype>(accent, out var prototype))
                return message;

            if (prototype.Words == null)
                return message;
